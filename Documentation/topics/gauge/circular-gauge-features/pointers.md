...
This behavior can be changed by setting the [PointerDirection](xref:@ActiproUIRoot.Controls.Gauge.Primitives.CircularPointerBase.PointerDirection) property. The pointer can be set to only move clockwise or counter-clockwise, or to always take the shortest path to the new value.

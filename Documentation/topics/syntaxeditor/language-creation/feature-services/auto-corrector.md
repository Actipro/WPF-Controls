...
The method is virtual, and its default implementation looks to see if the token is an [IMergeableToken](xref:ActiproSoftware.Text.Lexing.IMergeableToken).  If so, it checks to see if the token didn't match its source lexical pattern in a case sensitive way.  In that scenario, it returns the [IMergeableToken](xref:ActiproSoftware.Text.Lexing.IMergeableToken).[AutoCaseCorrectText](xref:ActiproSoftware.Text.Lexing.IMergeableToken.AutoCaseCorrectText) value.

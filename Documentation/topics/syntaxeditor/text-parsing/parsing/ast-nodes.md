...
| Member | Description |
|-----|-----|
| [Children](xref:ActiproSoftware.Text.Parsing.IAstNode.Children) Property | Gets the collection of child AST nodes. |
...
> [!NOTE]
> The [Children](xref:ActiproSoftware.Text.Parsing.Implementation.AstNodeBase.Children) property's enumerated results are typically yielded for performance. If multiple iterations through the enumerable are required while examining the AST node, it may be beneficial to use LINQ's `ToList` method on the enumerable to create a list of the child AST nodes.  This will create an `IList<IAstNode>` with a `Count` property and indexer.

...
- **Dynamic** - A [dynamic lexer](../text-parsing/lexing/dynamic-lexers.md) is used with the language.  This lexer type is mergeable and is the fastest way to get going.  It uses regular expression patterns to define how text is tokenized.
- **Programmatic (mergeable)** - A [mergeable programmatic lexer](../text-parsing/lexing/programmatic-lexers.md) is used with the language.  This lexer type is mergeable and uses a handwritten code-based lexer to tokenize text.  It is faster than a dynamic lexer.
- **Programmatic (non-mergeable)** - A [non-mergeable programmatic lexer](../text-parsing/lexing/programmatic-lexers.md) is used with the language.  This lexer type is not mergeable and uses a handwritten code-based lexer to tokenize text.  It provides the fastest performance.
...
To add states, select the **Programmatic Mergeable Lexer > Lexical States** node then click the **Add** button in the ribbon.  A dialog will appear asking you for the keys of the new states.  Enter one key per line, where each key conforms to standard identifier syntax since they will be used in code generation.  A new state is created for each key that you enter.

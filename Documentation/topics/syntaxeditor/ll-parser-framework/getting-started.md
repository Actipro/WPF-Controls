...
An [ITokenReader](xref:ActiproSoftware.Text.Parsing.LLParser.ITokenReader)-based class is necessary to provide tokens to the parsing framework.  For some languages with [mergeable lexers](../text-parsing/lexing/basic-concepts.md), it is sufficient to use the existing [MergeableTokenReader](xref:ActiproSoftware.Text.Parsing.LLParser.Implementation.MergeableTokenReader) class.  However, in non-mergeable lexer scenarios or scenarios where you wish to filter the tokens being supplied to the parser, you may need to subclass [TokenReaderBase](xref:ActiproSoftware.Text.Parsing.LLParser.Implementation.TokenReaderBase) or [MergeableTokenReader](xref:ActiproSoftware.Text.Parsing.LLParser.Implementation.MergeableTokenReader) for more control.

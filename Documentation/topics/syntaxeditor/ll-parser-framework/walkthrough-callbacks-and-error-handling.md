...
An error occurs when one or more terminals are expected by the parser (per the grammar) but a different token is found at the current location in the document.

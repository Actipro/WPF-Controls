...
If you are using a [mergeable lexer](../text-parsing/lexing/basic-concepts.md) to supply tokens to the parser, it may be sufficient to use the built-in [MergeableTokenReader](xref:ActiproSoftware.Text.Parsing.LLParser.Implementation.MergeableTokenReader) class since it fully implements [ITokenReader](xref:ActiproSoftware.Text.Parsing.LLParser.ITokenReader).  The [MergeableTokenReader](xref:ActiproSoftware.Text.Parsing.LLParser.Implementation.MergeableTokenReader) class will by default provide every token from a mergeable lexer to the parser.  If you have a mergeable lexer but would like to filter out certain token types, such as whitespace in a whitespace insensitive language, you can subclass [MergeableTokenReader](xref:ActiproSoftware.Text.Parsing.LLParser.Implementation.MergeableTokenReader) as shown in sample below.

If you are using a non-mergeable lexer then you should inherit [TokenReaderBase](xref:ActiproSoftware.Text.Parsing.LLParser.Implementation.TokenReaderBase) instead and implement its abstract [GetNextToken](xref:ActiproSoftware.Text.Parsing.LLParser.Implementation.TokenReaderBase.GetNextToken*) method, which simply returns the next token that will be consumed by the parser.

If you inherit from [MergeableTokenReader](xref:ActiproSoftware.Text.Parsing.LLParser.Implementation.MergeableTokenReader), your token reader class will need two things.  First, you will need a public constructor which takes an [ITextBufferReader](xref:ActiproSoftware.Text.ITextBufferReader) and an [IMergeableLexer](xref:ActiproSoftware.Text.Lexing.IMergeableLexer) as arguments.  Pass these along to the base constructor.  Second, you will need to override the [GetNextToken](xref:ActiproSoftware.Text.Parsing.LLParser.Implementation.MergeableTokenReader.GetNextToken*) member.

Here is an example token reader, based on [MergeableTokenReader](xref:ActiproSoftware.Text.Parsing.LLParser.Implementation.MergeableTokenReader), which discards all whitespace tokens.  The concepts here show how to filter tokens regardless of whether you inherit [MergeableTokenReader](xref:ActiproSoftware.Text.Parsing.LLParser.Implementation.MergeableTokenReader) or [TokenReaderBase](xref:ActiproSoftware.Text.Parsing.LLParser.Implementation.TokenReaderBase).  The `SimpleParsingTokenId` class contains token ID constants and was generated by the [Language Designer](../language-designer-tool/index.md) tool using our `Simple` language project.  You should have an equivalent class that was generated for your language.  This also makes the assumption that you have a `Whitespace` token defined in the language that was used to generate the `SimpleParsingTokenId` class.
...
/// </summary>
public class SimpleTokenReader : MergeableTokenReader {
	...
	public SimpleTokenReader(ITextBufferReader reader, IMergeableLexer rootLexer) : base(reader, rootLexer) {}
...
In this example code, we will create and return a [MergeableTokenReader](xref:ActiproSoftware.Text.Parsing.LLParser.Implementation.MergeableTokenReader) instance, but you could just as easily return a new instance of your own [ITokenReader](xref:ActiproSoftware.Text.Parsing.LLParser.ITokenReader) implementation.  The `SimpleLexer` and `SimpleClassificationTypeProvider` classes were generated by the [Language Designer](../language-designer-tool/index.md) tool using our `Simple` language project.  You should have equivalent classes that were generated for your language.
...
public class SimpleParser : LLParserBase {
	...
	public override ITokenReader CreateTokenReader(ITextBufferReader reader) {
		return new MergeableTokenReader(reader, new SimpleLexer(new SimpleClassificationTypeProvider()));
	}
}
```

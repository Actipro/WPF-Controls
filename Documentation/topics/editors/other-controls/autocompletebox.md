...
The suggestion list selects the first item by default, if the [CanAutoSelectFirstItem](xref:@ActiproUIRoot.Controls.Editors.AutoCompleteBox.CanAutoSelectFirstItem) property remains its default value of `true`.  Change that property to `false` to prevent auto-selection.  If you wish to set another item as the default selection, use the [SelectedItem](xref:@ActiproUIRoot.Controls.Editors.AutoCompleteBox.SelectedItem) property.  The [SelectedItemChanged](xref:@ActiproUIRoot.Controls.Editors.AutoCompleteBox.SelectedItemChanged) event is raised when this property is changed.

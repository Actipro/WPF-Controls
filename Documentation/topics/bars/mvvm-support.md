...
2. Instead of setting the [RibbonBackstage](xref:@ActiproUIRoot.Controls.Bars.RibbonBackstage).`ContentTemplate` property, add `DataTemplate` instances to the [RibbonBackstage](xref:@ActiproUIRoot.Controls.Bars.RibbonBackstage).`DataTemplates` collection with each data template's `x:DataType` attribute set to the related view model type.
}
@if (wpf) {

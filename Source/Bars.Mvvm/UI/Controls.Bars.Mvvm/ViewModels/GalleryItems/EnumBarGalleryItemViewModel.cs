...
public class EnumBarGalleryItemViewModel<TEnum> : BarGalleryItemViewModel<TEnum>
	...
	private static void ExtractAttributeData(TEnum value, out string? label, out string? category, out string? description, out int? order)
		=> ExtractAttributeData(FindField(value), out label, out category, out description, out order);
	...
	private static FieldInfo? FindField(TEnum value) {
		...
		return GetFields(enumType).FirstOrDefault(f => ((TEnum)f.GetValue(obj: null)!).Equals(value));
	}

	/// <summary>
	/// The <see cref="FieldInfo"/> for each value defined for an enumeration.
	/// </summary>
	...
	public static IEnumerable<EnumBarGalleryItemViewModel<TEnum>> CreateCollection() {
		...
		foreach (var field in GetFields(enumType)) {
			...
			try {
				...
				var value = (TEnum)field.GetValue(obj: null)!;
	...
	public void RefreshFromAttributes() {
		// Read attribute data
		ExtractAttributeData(Value, out var label, out var category, out var description, out int? order);

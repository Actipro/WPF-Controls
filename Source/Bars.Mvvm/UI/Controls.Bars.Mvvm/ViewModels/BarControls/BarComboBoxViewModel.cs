...
public class BarComboBoxViewModel : BarGalleryViewModel {
	...
	private string? _inlineImageSourcePath = string.Empty;
	...
	private string _textPath = nameof(IBarGalleryItemViewModel.Label);
	...
	/// <summary>
	/// The path to an <c>ImageSource</c> gallery item property, corresponding to optional inline image display.
	/// </summary>
	public string? InlineImageSourcePath {
		get => _inlineImageSourcePath;
		set => SetProperty(ref _inlineImageSourcePath, value);
	}
	...
	public virtual void SelectItemByTextMatch<T>(Func<T, string> getItemTextFunc, string text) where T : IBarGalleryItemViewModel {
		if (getItemTextFunc is null)
			throw new ArgumentNullException(nameof(getItemTextFunc));

		text ??= string.Empty;

		SelectItemByValueMatch<T>(i => text.Equals(getItemTextFunc(i) ?? string.Empty));
	...
	public virtual void SelectItemByValueMatch<T>(Func<T, bool> matchPredicate, Func<T, string> getMatchedItemTextFunc, string fallbackText) where T : IBarGalleryItemViewModel {
		if (getMatchedItemTextFunc is null)
			throw new ArgumentNullException(nameof(getMatchedItemTextFunc));

		var matchedItem = SelectItemByValueMatch(matchPredicate);

		if (matchedItem is not null) {
			Text = getMatchedItemTextFunc(matchedItem) ?? string.Empty;
			return;
		}
	...
	/// <summary>
	/// The path to a string gallery item property, corresponding to <see cref="Text"/> display and entry.
	/// </summary>
	public string TextPath {
		get => _textPath;
		set => SetProperty(ref _textPath, value);
	}

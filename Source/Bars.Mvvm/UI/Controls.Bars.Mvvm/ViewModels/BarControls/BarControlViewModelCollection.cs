...
public class BarControlViewModelCollection : IReadOnlyCollection<IHasKey>, IEnumerable<IHasKey>, IEnumerable {
	...
	private readonly Dictionary<string, Func<string, IHasKey>> _factories = [];
	private readonly IBarImageProvider? _imageProvider;
	private readonly HashSet<string> _reentryKeys = [];
	private readonly Dictionary<string, IHasKey> _viewModels = [];
	...
	/// <summary>
	/// Initializes an instance of the class with the specified <see cref="IBarImageProvider"/>.
	/// </summary>
	...
	protected virtual IHasKey Create(string key) {
		...
		if (
			_imageProvider is not null
			&& viewModel is IHasVariantImages imageViewModel
			&& imageViewModel.SmallImageSource is null
			&& imageViewModel.MediumImageSource is null
			&& imageViewModel.LargeImageSource is null
		) {
			imageViewModel.SmallImageSource = _imageProvider.GetImageSource(key, BarImageSize.Small);
			imageViewModel.MediumImageSource = _imageProvider.GetImageSource(key, BarImageSize.Medium);
			imageViewModel.LargeImageSource = _imageProvider.GetImageSource(key, BarImageSize.Large);
		}

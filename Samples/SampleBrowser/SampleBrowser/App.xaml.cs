...
public partial class App : Application {
	...
	/// </summary>
	public App() {
		_logger?.LogInformation("Initialize component");
		InitializeComponent();
	}
	...
	protected override void OnStartup(StartupEventArgs e) {
		...
		_logger?.LogInformation("Configuring ImageProvider ...");
		...
		_logger?.LogInformation("Configuring ThemeManager ...");
		...
		try {
			...
			_logger?.LogDebug("ThemeManager.AreNativeThemesEnabled = {0}", ThemeManager.AreNativeThemesEnabled);

			// Set a Metro Light theme
			ThemeManager.CurrentTheme = ThemeNames.MetroLight;
			_logger?.LogDebug("ThemeManager.CurrentTheme = {0}", ThemeManager.CurrentTheme);
		...
		_logger?.LogInformation("Configuring SyntaxEditor ...");
		...
		_logger?.LogDebug("AmbientParseRequestDispatcherProvider.Dispatcher = {0}", ActiproSoftware.Text.Parsing.AmbientParseRequestDispatcherProvider.Dispatcher?.GetType().FullName ?? "NULL");
		...
		var appDataPath = Path.Combine(
				...
				"Actipro Software",
				"WPF SampleBrowser Assembly Repository"
			);
		...
		_logger?.LogDebug(".NET Reflection Repository Path = {0}", appDataPath);
		...
		appDataPath = Path.Combine(
			Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
			"Actipro Software",
			"WPF SampleBrowser Python Package Repository"
		);
		...
		_logger?.LogDebug("Python Package Repository Path = {0}", appDataPath);

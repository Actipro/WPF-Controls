...
public class RichTextEditorRibbonViewModel : RibbonViewModel {
	...
	public RichTextEditorRibbonViewModel(BarManager barManager, RecentDocumentManager recentDocuments) {
		...
		RecentDocuments = recentDocuments ?? throw new ArgumentNullException(nameof(recentDocuments));

		// Create Application Button with Backstage
		ApplicationButton = CreateApplicationButton();
		Backstage = CreateBackstage(barManager, recentDocuments);
		...
		// Create toolbars
		QuickAccessToolBar = CreateQuickAccessToolBar(barManager);
		TabRowToolBar = CreateTabRowToolBar(barManager);

		// Create tabs
		Tabs.AddRange(CreateTabs(barManager));
		ContextualTabGroups.AddRange(CreateContextualTabGroups());
	}
	...
	/// <summary>
	/// Returns created instances of <see cref="RibbonContextualTabGroupViewModel"/> that will be used to initialize the
		...
		return new RibbonTabViewModel("Home") {
			...
			Groups = {
				...
				new RibbonGroupViewModel(BarControlKeys.GroupFont, "Font") {
					...
					Items = {
						new RibbonMultiRowControlGroupViewModel() {
							ThreeRowItemSortOrder = [0, 3, 2, 4, 1],
							Items = {
								new RibbonControlGroupViewModel() {
				...
				new RibbonGroupViewModel(BarControlKeys.GroupParagraph, "Paragraph") {
					...
					Items = {
						new RibbonMultiRowControlGroupViewModel() {
							ThreeRowItemSortOrder = [0, 1, 4, 5, 6, 2, 3],
							Items = {
								new RibbonControlGroupViewModel() {
									Items = {
	...
	/// <summary>
	/// Returns created instances of <see cref="RibbonTabViewModel"/> that will be used to initialize the
	...
	/// <summary>
	/// The <see cref="RecentDocumentManager"/> associated with the view model.
	/// </summary>

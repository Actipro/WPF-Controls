...
public class NumberingBarGalleryItemViewModel : BarGalleryItemViewModel<NumberingKind> {
	...
	public static IEnumerable<NumberingBarGalleryItemViewModel> CreateDefaultCollection() {
		return [
			new(NumberingKind.None),
			new(NumberingKind.ArabicNumeral) { Format = DotFormat },
			new(NumberingKind.ArabicNumeral) { Format = ParenthesisFormat },
			new(NumberingKind.UpperRomanNumeral),
			new(NumberingKind.LowerRomanNumeral),
			new(NumberingKind.UpperAlpha) { Format = DotFormat },
			new(NumberingKind.UpperAlpha) { Format = ParenthesisFormat },
			new(NumberingKind.LowerAlpha) { Format = DotFormat },
			new(NumberingKind.LowerAlpha) { Format = ParenthesisFormat },
		];
	}
	...
	/// <summary>
	/// The string format used for displaying the numbering.
	/// </summary>
	/// <value>
	...
	public override string ToString()
		=> $"{GetType().FullName}[Value='{Value}', Format='{Format}']";

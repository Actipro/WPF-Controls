...
public abstract class SampleControlBase : UserControl {
	...
	public static readonly DependencyProperty RecentDocumentManagerProperty
		= DependencyProperty.Register(nameof(RecentDocumentManager), typeof(RecentDocumentManager), typeof(SampleControlBase));
	...
	private void InitializeDynamicMenuItems(ItemsControl menu) {
		...
		switch (key) {
			...
			case BarControlKeys.OpenDocument:
				...
				if (openDocumentItems is not null) {
	...
	private static IList? ResolveMenuItemsList(ItemsControl? menu) {
		...
		return menu?.ItemsSource as IList
			?? menu?.Items;
	}
	...
	protected void NotifyMenuOpening(object sender, BarMenuEventArgs e) {
		...
		if (
			...
			InitializeDynamicMenuItems(menu);
		}
	}
	...
	/// <summary>
	/// The recent document manager associated with this control.
	/// </summary>

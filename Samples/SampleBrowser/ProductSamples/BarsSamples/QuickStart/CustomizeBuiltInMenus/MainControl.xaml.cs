...
public partial class MainControl {
	...
	private readonly ImageSource _customCommandSmallImage;
	private readonly ImageSource _customCommandLargeImage;
	...
	public MainControl() {
		...
		_customCommandLargeImage = ImageLoader.GetIcon("QuickStartGreen32.png");
	...
	private BarMenuItem CreateCustomActionBarMenuItem(object? owner) {
		var customActionMenuItem = new BarMenuItem() {
			...
			Description = $"This menu item was added programmatically on {DateTime.Today:d} at {DateTime.Now:T} using the Ribbon.MenuOpening event.",
			KeyTipText = "C",
			Label = $"Custom Action (Created @ {DateTime.Now:T})",
			LargeImageSource = _customCommandLargeImage,
			ScreenTipHeader = "Custom Action",
			SmallImageSource = _customCommandSmallImage,
			UseLargeSize = false,
		};
	...
	/// <summary>
	/// The command that will be associated with the custom menu item.
	/// </summary>
	private ICommand CustomCommand {
		get => _customCommand ??= new DelegateCommand<object>(p => {
			var message = (p is null)
				? "You clicked the programmatically-added custom menu item."
				: $"You clicked the programmatically-added custom menu item for:{Environment.NewLine}{Environment.NewLine}{p}";
			MessageBox.Show(message, "Custom Action", MessageBoxButton.OK, MessageBoxImage.Information);
		});
	}
	...
	private void CustomizeMenu(BarMenuEventArgs e) {
		...
		if (e.Kind == BarMenuKind.ControlContextMenu) {
			foreach (var menuItem in menu.Items.OfType<BarMenuItem>()) {
				if (
					menuItem.Command == ribbon.AddToQuickAccessToolBarCommand
					|| menuItem.Command == ribbon.RemoveFromQuickAccessToolBarCommand
				) {
	...
	/// <summary>
	/// Returns a description for a menu kind.
	/// </summary>
	/// <param name="kind">The menu kind to examine.</param>
	private static string GetMenuKindDescription(BarMenuKind kind) {
		return kind switch {
			BarMenuKind.ControlContextMenu => "Control Context Menu",
			BarMenuKind.RibbonOptionsButtonMenu => "Options Menu",
			BarMenuKind.RibbonQuickAccessToolBarCustomizeButtonMenu => "Quick Access Toolbar Customize Menu",
			BarMenuKind.RibbonTabItemOverflowButtonMenu => "Tab Overflow Menu",
			BarMenuKind.RibbonGroupOverflowButtonMenu => "Group Overflow Menu",
			_ => "Unknown Menu"
		};
	}
	...
	private void OnRibbonMenuOpening(object sender, BarMenuEventArgs e) {
		...
		if (
			e.Kind == BarMenuKind.PopupButtonMenu

...
namespace ActiproSoftware.ProductSamples.BarsSamples.QuickStart.GettingStarted.Step03 {
	...
	public class SampleBarManager {
		...
		private static ImageSource CreateBitmapImage(BarImageOptions options, string smallFileName, string largeFileName) {
			...
			//		to load an ImageSource from files stored as resources with the sample application, but any
			//		ImageSource can be used.
			//		
		...
		private void RegisterControlViewModels() {
			...
			ControlViewModels.Register(
				...
				key => new BarButtonViewModel(					// Delegate factory creates a new instance of the view model
					...
					ApplicationCommands.Help) {					// The command associated with the button
						Description = "Open application help."	// The ScreenTip description (i.e. ToolTip) for the control
					});

			//	SAMPLE NOTE 3.6:
			//		The BarButtonViewModel has properties for SmallImageSource, MediumImageSource, and LargeImageSource
			//		that can be explicitly defined when created. Since we are using an ImageProvider that has images
			//		registered for the Help control, those properties will be automatically populated by the
			//		BarControlViewModelCollection when creating a view model. Images can only be automatically assigned
			//		if the key used to register the images match the key used to register the control!
			//		
			//		If desired, the ImageProvider does not have to be used and appropriate values can be assigned to the
			//		image properties when the view model is created.
			//		
			//		Even if the ImageProvider is used, images will only be automatically assigned if the created view
			//		model has a NULL ImageSource for each image property. Otherwise it assumes images are being explicitly
			//		defined by the view model. This means even if you use ImageProvider you can override the automatic
			//		assignment of images by explicitly defining an ImageSource for any of the image properties.
		}

...
public partial class SampleMvvmControl : UserControl {
	...
	public SampleMvvmControl() {
		...
		ScreenTipService.Current.ScreenTipOpening += OnScreenTipOpening;
	...
	private void OnScreenTipOpening(object? sender, ToolTipEventArgs e) {
		if (
			...
			&& ToolTipService.GetToolTip(element) is ScreenTip screenTip
		) {
			...
			if (key == ButtonKeyDynamicScreenTip) {
				...
				screenTip.Content = FindResource("ComplexScreenTipContent");
				screenTip.Footer = FindResource("ComplexScreenTipFooterContent");
			}
			else if (key == ButtonKeyFooterHelpContext) {
				// Assign the Footer using a XAML resource
				screenTip.Footer = FindResource("FooterHelpContextContent");
			}
		}
	}

...
public partial class MainControl {
	...
	public static readonly DependencyProperty ValueProperty
		= DependencyProperty.Register(nameof(Value), typeof(double), typeof(MainControl), new FrameworkPropertyMetadata(defaultValue: 42.0));
	...
	private void OnLoaded(object sender, EventArgs e) {
		...
		targetPanel.Children.Insert(0, circularGauge);

		// Create and add a CircularScale to the CircularGauge
		var circularScale = new CircularScale() {
			Radius = 75,
			StartAngle = 30,
			SweepAngle = 330,
			BarExtent = 1
		...
		var circularTickSet = new CircularTickSet() {
			MajorInterval = 10,
			MinorInterval = 2
		};
		...
		var circularPointerNeedle = new CircularPointerNeedle() {
		...
		circularPointerNeedle.SetBinding(CircularPointerNeedle.ValueProperty, new Binding(nameof(Value)) {
			Source = this,

	...
	public static class ConvertItemHelper {
		...
		private static string GetPathEntry(XmlElement element) {
			...
			if (element.Name == "MyRootElement")
				return "Root";
			else if (element.Name == "MyFolderElement" && 0 != element.Attributes.Count)
				return element.Attributes["FolderName"].Value;
			else if (element.Name == "MySubfolderElement" && 0 != element.Attributes.Count)
				return element.Attributes["SubfolderName"].Value;

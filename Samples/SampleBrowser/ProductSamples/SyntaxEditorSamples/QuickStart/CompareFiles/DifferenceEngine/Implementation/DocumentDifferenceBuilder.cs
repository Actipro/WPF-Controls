...
public class DocumentDifferenceBuilder : IDocumentDifferenceBuilder {
	...
	private readonly struct ComparableItem(int index, string text, bool ignoreWhiteSpace) {
		...
		/// <summary>
		/// The calculated hash code for the item.
		/// </summary>
		public int Hash { get; } = CalculateHash(text, ignoreWhiteSpace);

		/// <summary>
		/// The zero-based position of the item within a collection.
		/// </summary>
		public int Index { get; } = index;

		/// <summary>
		/// The length of the item's text.
		/// </summary>
		public int Length
			=> Text?.Length ?? 0;
	...
	IDifferenceCollection? IDocumentDifferenceBuilder.NewDifferences
		=> NewDifferences;

	IDifferenceCollection? IDocumentDifferenceBuilder.OldDifferences
		=> OldDifferences;
	...
	private static IEnumerable<ComparableItem> ItemizeDocument(string documentText, bool ignoreWhiteSpace) {
		...
		if ((documentText.Length > 0) && documentText[documentText.Length - 1].IsLineTerminator())

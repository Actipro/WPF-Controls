...
public class CompareFilesImaginaryLinesTagger(IEditorView view) : TaggerBase<IIntraLineSpacerTag>("CompareFilesImaginaryLines", orderings: null, view.SyntaxEditor.Document, isForLanguage: true) {
	...
	private readonly ConcurrentDictionary<int, ImaginaryLineDifferenceTag> _imaginaryLineAnchors = [];
	...
	private void InvalidateAllTags(bool updateDefaultLineHeight) {
		#if DEBUG_TAGS
		Debug.WriteLine($"{GetType().Name}.{nameof(InvalidateAllTags)}");
		#endif

		// Update the default line height of each tag
		if (updateDefaultLineHeight) {
			var keys = _imaginaryLineAnchors.Keys.ToArray();
			foreach (var key in keys) {
				if (_imaginaryLineAnchors.TryGetValue(key, out var tag)) {
					// Quit if the DefaultLineHeight does not need to change
					if (tag.DefaultLineHeight == _view.DefaultLineHeight)
						break;
					tag.DefaultLineHeight = _view.DefaultLineHeight;
				}
			}
		}
			...
			for (var i = 0; i < differences.Count; i++) {
				if (differences[i].Kind == DifferenceKind.Imaginary) {
					...
					var tag = new ImaginaryLineDifferenceTag() {
						DefaultLineHeight = _view.DefaultLineHeight,
						LineCount = lineCount,
					};
					...
					if (lastRealDiffIndex >= 0) {
						...
						_imaginaryLineAnchors[lastRealLineIndex] = tag;

						#if DEBUG_TAGS
						Debug.WriteLine($"{GetType().Name}.{nameof(ApplyDifferences)} :: {lineCount} imaginary line(s) anchored below real document line index {lastRealLineIndex} (diff index {lastRealDiffIndex}).");
						#endif
					}
					else {
						...
						_imaginaryLineAnchors[FirstLineTopTagIndex] = tag;

						#if DEBUG_TAGS
						Debug.WriteLine($"{GetType().Name}.{nameof(ApplyDifferences)} :: {lineCount} imaginary line(s) anchored above first real line.");
						#endif
					}
				}
				else {
	...
	public override IEnumerable<TagSnapshotRange<IIntraLineSpacerTag>> GetTags(NormalizedTextSnapshotRangeCollection snapshotRanges, object? parameter) {
		...
		if (snapshotRanges is not null) {
			...
			foreach (var snapshotRange in snapshotRanges) {
				...
				if (lineIndex == 0) {
					#if DEBUG_TAGS
					Debug.WriteLineIf(showDebug, $"{GetType().Name}.{nameof(GetTags)} :: Testing first line as imaginary anchor for top tags (total anchor lines = {_imaginaryLineAnchors.Count})");
					#endif
					if (_imaginaryLineAnchors.TryGetValue(FirstLineTopTagIndex, out var topSpacerTag)) {
						#if DEBUG_TAGS
						Debug.WriteLineIf(showDebug, $"{GetType().Name}.{nameof(GetTags)} :: First line tagged as anchor for top {topSpacerTag.LineCount} imaginary lines.");
						#endif

						// Yield the tag
						yield return new TagSnapshotRange<IIntraLineSpacerTag>(
							TextSnapshotRange.FromSpan(snapshotRange.Snapshot, snapshotRange.StartLine.StartOffset, snapshotRange.StartLine.Length),
							topSpacerTag
						);
					}
				}
				...
				#if DEBUG_TAGS
				Debug.WriteLineIf(showDebug, $"{GetType().Name}.{nameof(GetTags)} :: Testing line index {lineIndex} as imaginary line anchor (total anchor lines = {_imaginaryLineAnchors.Count})");
				#endif
				if (_imaginaryLineAnchors.TryGetValue(lineIndex, out var spacerTag)) {
					#if DEBUG_TAGS
					Debug.WriteLineIf(showDebug, $"{GetType().Name}.{nameof(GetTags)} :: Line index {lineIndex} tagged as anchor for {spacerTag.LineCount} imaginary lines.");
					#endif

...
public class ColorPreviewAdornmentManager(IEditorView view) : DecorationAdornmentManagerBase<IEditorView, ColorPreviewTag>(view, _layerDefinition) {
	...
	private void OnDrawHighlightAdornment(TextViewDrawContext context, IAdornment adornment) {
		if (adornment.Tag is ColorPreviewTag tag) {
	...
	protected override void AddAdornment(AdornmentChangeReason reason, ITextViewLine viewLine, TagSnapshotRange<ColorPreviewTag> tagRange, TextBounds bounds) {
		...
		var adornmentBounds = new Rect(
			Math.Round(bounds.Left, MidpointRounding.AwayFromZero),
			Math.Round(bounds.Top, MidpointRounding.AwayFromZero),
			Math.Round(bounds.Width, MidpointRounding.AwayFromZero),
			Math.Round(bounds.Height, MidpointRounding.AwayFromZero)
		);

		// Add the adornment to the layer
		AdornmentLayer.AddAdornment(reason, OnDrawHighlightAdornment, adornmentBounds, tagRange.Tag, viewLine, tagRange.SnapshotRange, TextRangeTrackingModes.ExpandBothEdges, removedCallback: null);
	}

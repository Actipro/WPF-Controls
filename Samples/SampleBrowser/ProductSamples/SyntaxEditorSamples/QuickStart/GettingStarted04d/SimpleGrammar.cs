...
public class SimpleGrammar : Grammar {
	...
	private readonly NonTerminal _statement;
	...
	public SimpleGrammar() : base("Simple") {
		...
		Root = new NonTerminal("CompilationUnit");
		...
		_statement = new NonTerminal("Statement") { ErrorAlias = "Statement" };
		...
		Root.Production = functionDeclaration.OnError(AdvanceToDefaultState).ZeroOrMore().SetLabel("decl")
		...
		_statement.Production = block["stmt"] > AstFrom("stmt")
			| emptyStatement > null
		...
		blockChildStatement.Production = _statement["stmt"] > AstFrom("stmt");
	...
	internal static bool BinaryOperatorKindCheck(BinaryOperatorExpression expr1, BinaryOperatorExpression expr2, params OperatorKind[] operators) {
		if ((expr1 is not null) && (expr2 is not null) && (operators is not null)) {
	...
	private bool CanMatchBlockChildStatement(IParserState state) {
		...
		return !state.TokenReader.IsAtEnd
			&& state.TokenReader.LookAheadToken.Id != SimpleTokenId.CloseCurlyBrace;
	}

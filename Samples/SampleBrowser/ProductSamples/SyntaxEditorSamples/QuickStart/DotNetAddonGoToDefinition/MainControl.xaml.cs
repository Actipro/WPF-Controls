...
public partial class MainControl : UserControl {
	...
	public MainControl() {
		...
		_goToDefinitionService = new GoToDefinitionService();
		_goToDefinitionService.OpenEditors.Add(codeEditor);
		selectDefinitionCheckBox.IsChecked = _goToDefinitionService.IsWordSelectedOnNavigation;

		// Register the GoToDefinitionService on the language so it can be resolved by other types
		language.RegisterService(_goToDefinitionService);
	...
	private void OnSelectDefinitionCheckBoxIsCheckedChanged(object sender, RoutedEventArgs e) {
		if (_goToDefinitionService != null)
			_goToDefinitionService.IsWordSelectedOnNavigation = (selectDefinitionCheckBox.IsChecked == true);
	}

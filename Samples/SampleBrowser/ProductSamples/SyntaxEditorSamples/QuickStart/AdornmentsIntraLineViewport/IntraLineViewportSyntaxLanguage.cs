...
public class IntraLineViewportSyntaxLanguage : CSharpSyntaxLanguage {
	...
	public IntraLineViewportSyntaxLanguage() {
		...
		RegisterService(new AdornmentManagerProvider<IntraLineViewportAdornmentManager>(typeof(IntraLineViewportAdornmentManager)));

		// Register a tagger provider on the language as a service that can create IntraLineViewportTag objects
		RegisterService(new TextViewTaggerProvider<IntraLineViewportTagger>(typeof(IntraLineViewportTagger)));
	}

...
public class TagStyleTransitionSyntaxLanguage : SyntaxLanguage {
	...
	public TagStyleTransitionSyntaxLanguage() : base("Xml") {
		...
		var xmlNameClassificationType = AmbientHighlightingStyleRegistry.Instance["XmlName"];
		var xmlDelimiterClassificationType = AmbientHighlightingStyleRegistry.Instance["XmlDelimiter"];
		...
		using (parentLexer.CreateChangeBatch()) {
			...
			var lexicalState = new DynamicLexicalState(0, "ScriptStartTag") {
				DefaultTokenKey = "StartTagText",
				DefaultClassificationType = xmlNameClassificationType
			};
			...
			var transitionLexicalScope = new DynamicLexicalScope {
				EndLexicalPatternGroup = new DynamicLexicalPatternGroup(DynamicLexicalPatternType.Explicit, "EndTagStartDelimiter", xmlDelimiterClassificationType)
			};
